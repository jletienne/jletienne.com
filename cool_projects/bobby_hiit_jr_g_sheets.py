...
def get_final_sheet_row_number():
    ...
    result = sheet.values().get(spreadsheetId = '1EeG0NHoxDU4Wy_Kw7qQdhEYlCw5Xu8drEzL-yirSbKs', range='fitbod_data!A:A').execute()
...
SPREADSHEET_ID = '1EeG0NHoxDU4Wy_Kw7qQdhEYlCw5Xu8drEzL-yirSbKs' # Get this one from the link in browser
worksheet_name = 'fitbod_data'
...
def get_last_fitbod_date():

    sheet = API.spreadsheets()
    result = sheet.values().get(spreadsheetId = '1EeG0NHoxDU4Wy_Kw7qQdhEYlCw5Xu8drEzL-yirSbKs', range='max_fitbod_date!A2').execute()
    values = result.get('values', [])

    return values[0][0]

def get_new_fitbod_max_date_df(final):
    max(final['Date'])
    max_date_uts = max(final['Date'])
    return pd.DataFrame([{'Date': max_date}])
